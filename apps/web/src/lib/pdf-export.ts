...
export async function exportComplianceCheckToPDF(
  ...
  return new Promise((resolve, reject) => {
    ...
    doc.fillColor(overallCompliant ? 'green' : 'red');
    doc.fontSize(12).text(overallCompliant ? 'COMPLIANT' : 'NON-COMPLIANT');
    doc.fillColor('black');
    ...
    checks.forEach((check, index) => {
      ...
      doc.fillColor(check.status === 'COMPLIANT' ? 'green' : check.status === 'NON_COMPLIANT' ? 'red' : 'orange');
      doc.fontSize(10).text(`   Status: ${check.status}`, { indent: 20 });
      doc.fillColor('black');
      ...
      if (check.gaps && check.gaps.length > 0) {
        doc.fillColor('red');
        doc.fontSize(9).text(`   Gaps: ${check.gaps.join('; ')}`, { indent: 20 });
        doc.fillColor('black');
      }
...
export async function exportGapAnalysisToPDF(
  ...
  return new Promise((resolve, reject) => {
    ...
    doc.fillColor('red');
    doc.fontSize(12).text(`High Priority: ${analysis.summary.high}`);
    doc.fillColor('orange');
    doc.fontSize(12).text(`Medium Priority: ${analysis.summary.medium}`);
    doc.fillColor('green');
    doc.fontSize(12).text(`Low Priority: ${analysis.summary.low}`);
    doc.fillColor('black');
    doc.moveDown();
    ...
    if (analysis.gaps.length > 0) {
      ...
      analysis.gaps.forEach((gap, index) => {
        ...
        doc.fillColor(color);
        doc.fontSize(12).text(`${index + 1}. ${gap.title}`, { indent: 20 });
        doc.fillColor('black');
        doc.fontSize(10).text(`   Type: ${gap.type}`, { indent: 20 });
        doc.fillColor(color);
        doc.fontSize(10).text(`   Severity: ${gap.severity.toUpperCase()}`, { indent: 20 });
        doc.fillColor('black');
...
export async function exportDocumentListToPDF(
  ...
  return new Promise((resolve, reject) => {
    ...
    // Footer
    doc.fontSize(8).text(
      `Generated on ${new Date().toLocaleString('en-US')} | Total documents: ${documents.length}`,
      { align: 'center' }
    );
